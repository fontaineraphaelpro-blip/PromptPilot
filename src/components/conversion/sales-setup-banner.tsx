...
import { isPaymentsLive, isSalesMode } from "@/lib/sales-mode";

/** Alerte admin si mode vente actif mais Stripe public non configuré. */
export function SalesSetupBanner() {
  if (!isSalesMode() || isPaymentsLive()) return null;

  return (
    <div className="border-b border-amber-500/30 bg-amber-500/15 px-4 py-2 text-center text-xs sm:text-sm text-amber-100">
      Mode vente actif — configure{" "}
